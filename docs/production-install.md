...
- Providers you will use:
  ...
  - Bitties Proxy: `BITTIES_PROXY_BASE` + (`BITTIES_PROXY_TOKEN` or `BITTIES_PROXY_USERNAME`/`BITTIES_PROXY_PASSWORD`), `BITTIES_PROXY_THRESHOLD_BTC` (default `0.0011`; values above `NICEHASH_MIN_START_AMOUNT_BTC` are capped), plus `BITTIES_PROXY_TOTAL_HASHRATE_TH` (for share-based weight) and optional `BITTIES_PROXY_WEIGHT_SCALE` (default `100`)
